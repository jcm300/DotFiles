...
cp /mnt/pacman.conf /etc/pacman.conf
...
#Install packages
cat "/mnt/packages" | xargs yay -S --noconfirm
...
#Lock Screen
mv /mnt/configs/lockScreen/lockScreen.service /etc/systemd/system/
systemctl start lockScreen.service
systemctl enable lockScreen.service

#Remove desktop folder from home
mv /mnt/configs/removeDesktopFolder/user-dirs.dirs ~/.config/
